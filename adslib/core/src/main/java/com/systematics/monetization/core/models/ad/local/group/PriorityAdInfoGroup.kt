...
data class PriorityAdInfoGroup(
    val priorityAdUnits: List<AdInfo>,
    val defaultAdUnit: List<AdInfo>,
    ...
    override fun validateAdInfoGroup(): PriorityAdInfoGroup {
        return this.copy(
            priorityAdUnits = priorityAdUnits.map { it.mapValidated() },
            defaultAdUnit = defaultAdUnit.map { it.mapValidated() },
        )
    }
}