package com.systematics.photocleaner.swipedelete.data.swipedelete
...
data class DecisionRecord(val media: SwipeDeleteMedia, val decision: MediaDecision, val sequence: Int)
data class CleanupSession(
    val id: Long,
    val scopeKey: String,
    val title: String,
    val partial: Boolean,
    val media: List<SwipeDeleteMedia>,
    val decisions: List<DecisionRecord>,
) {
    val current: SwipeDeleteMedia? get() {
...
class SwipeDeleteController(private val context: Context) {
    private val resolver = context.contentResolver
    private val prefs = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
    private val db = SwipeDeleteDb(context)
    private val scope = CoroutineScope(SupervisorJob() + Dispatchers.IO)
    private val _state = MutableStateFlow(loadInitial())
    val state: StateFlow<SwipeDeleteState> = _state.asStateFlow()
    private val monthMedia = mutableMapOf<String, List<SwipeDeleteMedia>>()
    ...
    private fun loadInitial(): SwipeDeleteState {
    ...
    private fun queryImages(): List<SwipeDeleteMedia> {
        ...
        val items = mutableListOf<SwipeDeleteMedia>()
    ...
    companion object {
        ...
        private const val PREFS = "swipedelete_state"
...
private class SwipeDeleteDb(context: Context) : SQLiteOpenHelper(context, "swipedelete.db", null, 1) {
    ...
    fun decisions(id: Long): List<DecisionRecord> = readableDatabase.rawQuery(
        ...
        buildList {
            while (c.moveToNext()) add(DecisionRecord(
                SwipeDeleteMedia(c.getString(0), c.getLong(2), if (c.isNull(3)) null else c.getLong(3), c.getInt(4), c.getInt(5)),
                MediaDecision.valueOf(c.getString(1)), c.getInt(6)
            ))
        }
    }
    fun addDecision(id: Long, media: SwipeDeleteMedia, kind: MediaDecision, sequence: Int): Int {
