...
import com.systematics.photocleaner.swipedelete.data.swipedelete.ThemeMode
import com.systematics.photocleaner.swipedelete.utils.core.getAppVersionCode
import com.systematics.photocleaner.swipedelete.utils.core.sendFeedbackEmail
import com.systematics.photocleaner.swipedelete.utils.providers.LocalAppLogEvents
import org.koin.compose.koinInject

@Composable
fun SwipeDeleteSettingsScreen(
    ...
    Column(Modifier.fillMaxSize().background(p.app).systemBarsPadding()){
        SwipeDeleteHeader("Photo Cleaner","Settings",{events.loadEvents("settings_scr_back_clck");onBack()})
        Column(Modifier.weight(1f).verticalScroll(rememberScrollState()).padding(18.dp,2.dp,18.dp,28.dp)){
            ...
            SettingsRow(R.drawable.ic_star,"Rate Photo Cleaner","Open Google Play",{events.loadEvents("settings_scr_rate_clck");onRate()})
            Column(Modifier.fillMaxWidth().padding(top=24.dp),horizontalAlignment=Alignment.CenterHorizontally){
                SwipeDeleteWord(size=20.sp);Kicker("Version ${BuildConfig.VERSION_NAME} · Made for your camera roll",Modifier.padding(top=5.dp))
            }
        }
    }
}
...
@Composable private fun SettingsRow(icon:Int,label:String,sub:String,onClick:()->Unit,danger:Boolean=false){
    ...
    HardSurface(Modifier.fillMaxWidth().height(72.dp).padding(bottom=8.dp).clickable(onClick=onClick),radius=SwipeDeleteDimens.radiusMd,
        shadowX=0.dp,shadowY=0.dp,borderWidth=2.dp,contentPadding=PaddingValues(12.dp)){
        Row(Modifier.fillMaxSize(),verticalAlignment=Alignment.CenterVertically){
            Box(Modifier.size(40.dp).background(if(danger)p.gone.copy(alpha=.18f) else p.inset,RoundedCornerShape(SwipeDeleteDimens.radiusSm)),contentAlignment=Alignment.Center){
            ...
            Spacer(Modifier.width(12.dp));Column(Modifier.weight(1f)){SwipeDeleteText(label,color=if(danger)p.gone else p.textStrong,fontWeight=FontWeight.ExtraBold,fontSize=14.sp);SwipeDeleteText(sub,color=p.textMuted,fontSize=12.sp)}
            SwipeDeleteText("›",color=p.textFaint,fontSize=22.sp)
        }
    }
}
@Composable private fun SettingsToggle(icon:Int,label:String,sub:String,checked:Boolean,onChange:(Boolean)->Unit){
    val p=LocalSwipeDeletePalette.current
    HardSurface(Modifier.fillMaxWidth().height(72.dp).padding(bottom=8.dp),radius=SwipeDeleteDimens.radiusMd,shadowX=0.dp,shadowY=0.dp,borderWidth=2.dp,contentPadding=PaddingValues(12.dp)){
        Row(Modifier.fillMaxSize(),verticalAlignment=Alignment.CenterVertically){
            Box(Modifier.size(40.dp).background(p.inset,RoundedCornerShape(SwipeDeleteDimens.radiusSm)),contentAlignment=Alignment.Center){
                androidx.compose.foundation.Image(painterResource(icon),null,Modifier.size(21.dp),colorFilter=ColorFilter.tint(p.keep))
            }
            Spacer(Modifier.width(12.dp));Column(Modifier.weight(1f)){SwipeDeleteText(label,color=p.textStrong,fontWeight=FontWeight.ExtraBold,fontSize=14.sp);SwipeDeleteText(sub,color=p.textMuted,fontSize=12.sp)}
    ...
    HardSurface(
        ...
        Column(Modifier.fillMaxWidth()) {
            Row(
                ...
                Box(
                    Modifier.size(40.dp).background(p.inset, RoundedCornerShape(SwipeDeleteDimens.radiusSm)),
            ...
            Box(
                Modifier
                    .fillMaxWidth()
                    .height(42.dp)
                    .clip(RoundedCornerShape(SwipeDeleteDimens.radiusSm))
                    .background(p.inset)
                    .border(2.dp, p.borderSoft, RoundedCornerShape(SwipeDeleteDimens.radiusSm))
                    .padding(3.dp)
            ) {
                Row(
                    ...
                    options.forEach { (mode, label) ->
                        ...
                        Box(
                            Modifier
                                .weight(1f)
                                .fillMaxHeight()
                                .clip(RoundedCornerShape(SwipeDeleteDimens.radiusXs))
                                .background(bg)
                                .border(if (selected) 2.dp else 0.dp, border, RoundedCornerShape(SwipeDeleteDimens.radiusXs))
                                .clickable {
...
fun FeedbackScreen(onBack:()->Unit){
    ...
    val p=LocalSwipeDeletePalette.current;val context=LocalContext.current;val events=LocalAppLogEvents.current
    ...
    Column(Modifier.fillMaxSize().background(p.app).systemBarsPadding()){
        SwipeDeleteHeader("Support","Send feedback",onBack)
            ...
            Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(8.dp)){
                listOf("Idea","Problem","Other").forEach{value->
                    ...
                    Box(
                        ...
                        SwipeDeleteText(value,color=if(selected)swipedeleteContentColorFor(p.keep) else p.textMuted,fontWeight=FontWeight.Black)
                    }
                }
            }
            Spacer(Modifier.height(14.dp))
            BasicTextField(message,{message=it.take(4000);error=false},Modifier.fillMaxWidth().heightIn(min=140.dp).background(p.card,RoundedCornerShape(SwipeDeleteDimens.radiusMd)).border(3.dp,if(error)p.gone else p.border,RoundedCornerShape(SwipeDeleteDimens.radiusMd)).padding(14.dp),
                textStyle=TextStyle(color=p.textStrong,fontFamily=SwipeDeleteTypography.ui.fontFamily,fontSize=15.sp),decorationBox={inner->if(message.isBlank())SwipeDeleteText("Tell us what you think…",color=p.textFaint);inner()})
            if(error)SwipeDeleteText("Write a message before sharing.",Modifier.padding(top=7.dp),color=p.gone,fontWeight=FontWeight.Bold)
        }
        Column(Modifier.fillMaxWidth().background(p.appRaised).padding(18.dp,10.dp)){
            KButton("Share feedback",{
                ...
                context.sendFeedbackEmail(
                    email = "syed.muhammad.shahroze@gmail.com",
...
fun RateUsScreen(onBack:()->Unit,onFeedback:()->Unit){
    AnalyticsLaunch("rate_scr_launch");val p=LocalSwipeDeletePalette.current;val context=LocalContext.current;var stars by rememberSaveable{mutableIntStateOf(5)};val events=LocalAppLogEvents.current
    StateScreen(R.drawable.ic_app_logo,"A small favor","Enjoying Photo Cleaner?","A quick rating helps other people find a cleaner that respects them.",p.reward,iconTint=null){
        Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.Center){(1..5).forEach{s->Box(Modifier.size(52.dp).clickable{stars=s},contentAlignment=Alignment.Center){SwipeDeleteText("★",color=if(s<=stars)p.reward else p.cardRaised,fontSize=38.sp)}}}
...
fun PermissionDeniedScreen(permanent:Boolean,onRetry:()->Unit,onSelected:()->Unit,onPrivacy:()->Unit,onNotNow:()->Unit){
    AnalyticsLaunch("denied_scr_launch");val p=LocalSwipeDeletePalette.current;val context=LocalContext.current
    StateScreen(R.drawable.ic_lock,"Photo access needed",if(permanent)"Allow access in Settings" else "Photo Cleaner can't see your photos yet",
...
@Composable
fun ResumeSessionScreen(onResume:()->Unit,onRestart:()->Unit,onMonths:()->Unit,onReview:()->Unit,controller:SwipeDeleteController=koinInject()){
    AnalyticsLaunch("resume_scr_launch");val state by controller.state.collectAsStateWithLifecycle();val s=state.session;val p=LocalSwipeDeletePalette.current
    StateScreen(R.drawable.ic_undo,"Progress restored","Continue where you left off",
        "Photo Cleaner saved ${s?.decisions?.size?:0} decisions from ${s?.title?:"this cleanup"} on this device. No deletion has started.",p.reward){
