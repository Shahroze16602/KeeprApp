    ...
    Column(Modifier.fillMaxSize().background(p.app).systemBarsPadding()) {
        ...
        if (tab == MediaDecision.Delete) {
            Row(Modifier.fillMaxWidth().padding(horizontal = 18.dp, vertical = 3.dp), verticalAlignment = Alignment.CenterVertically) {
                KeeprText("Est. ${deleteBytes.bytesLabel()} freed · tap a photo to keep it instead", color = p.textMuted, fontSize = 12.sp, fontWeight = FontWeight.Bold)
            }
        }
        LazyVerticalGrid(
...
@Composable
fun DeletionConfirmationScreen(onBack: () -> Unit, onConfirm: () -> Unit, controller: KeeprController = koinInject()) {
    ...
    val batches = (count + KeeprController.PLATFORM_DELETE_LIMIT - 1) / KeeprController.PLATFORM_DELETE_LIMIT
    val p = LocalKeeprPalette.current
    val events = com.systematics.keepr.utils.providers.LocalAppLogEvents.current
    Box(Modifier.fillMaxSize().background(Color.Black.copy(alpha=.72f)).systemBarsPadding(), contentAlignment = Alignment.BottomCenter) {
        Column(
            ...
            Spacer(Modifier.height(18.dp))
            HardSurface(Modifier.size(78.dp),radius=KeeprDimens.radiusLg,background=p.gone,shadowX=5.dp,shadowY=6.dp) {
...
fun DeletionProgressScreen(
    ...
    if(error!=null) {
        StateScreen(R.drawable.ic_warning,"Deletion paused","Keepr couldn't continue",error!!,p.gone){
    ...
    Column(Modifier.fillMaxSize().background(p.app).systemBarsPadding().padding(30.dp),horizontalAlignment=Alignment.CenterHorizontally,verticalArrangement=Arrangement.Center){
        ProgressRing(pct,size=150.dp,color=p.gone,stroke=14.dp){
            KeeprText((result.confirmed+result.unresolved).toString(),style=KeeprTypography.numeral,color=p.textStrong,fontSize=42.sp,fontWeight=FontWeight.Black)
        }
...
@Composable
fun CompletionScreen(onMonths:()->Unit,onRate:()->Unit,onPartial:()->Unit,controller:KeeprController=koinInject()){
    ...
    Box(Modifier.fillMaxSize().background(Brush.radialGradient(listOf(p.keep.copy(alpha=.22f),p.app),radius=1100f))){
        ...
        Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(8.dp)){
            ...
            KButton("Rate Keepr",{events.loadEvents("complete_scr_rate_clck");onRate()},Modifier.weight(1f),KButtonStyle.Ghost,icon=painterResource(R.drawable.ic_star))
        }
    }
    }
}
...
@Composable
private fun CompletionConfetti(p: KeeprPalette) {
    if (LocalKeeprReducedMotion.current) return
...
@Composable private fun CompletionStat(icon:String,value:Int,label:String,tone:Color,modifier:Modifier=Modifier){
    val p=LocalKeeprPalette.current
    HardSurface(modifier.height(112.dp),radius=KeeprDimens.radiusLg,shadowX=3.dp,shadowY=3.dp,contentPadding=PaddingValues(10.dp)){
        Column(Modifier.fillMaxSize(),horizontalAlignment=Alignment.CenterHorizontally,verticalArrangement=Arrangement.Center){
            KeeprText(icon,color=tone,fontSize=20.sp,fontWeight=FontWeight.Black)
            KeeprText(value.toString(),style=KeeprTypography.numeral,color=p.textStrong,fontSize=27.sp,fontWeight=FontWeight.Black);Kicker(label)
        }
    }
}

@Composable
fun PartialDeletionScreen(onReview:()->Unit,onRetry:()->Unit,onMonths:()->Unit,controller:KeeprController=koinInject()){
    AnalyticsLaunch("partial_scr_launch")
    val state by controller.state.collectAsStateWithLifecycle();val r=state.deletion;val p=LocalKeeprPalette.current
    StateScreen(R.drawable.ic_warning,"Action needed","${r.confirmed} deleted · ${r.unresolved} unresolved",
        "Android confirmed some deletions, but other photos are still present or unavailable. Keepr never counts those as deleted.",p.gone){
