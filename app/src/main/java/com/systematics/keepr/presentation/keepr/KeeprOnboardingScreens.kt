...
private val KeeprLanguages = listOf(
    ...
    Column(Modifier.fillMaxSize().background(p.app).systemBarsPadding()) {
        Row(Modifier.fillMaxWidth().padding(18.dp, 10.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            Column(Modifier.padding(start = if (isFromSplash) 0.dp else 12.dp)) {
                Kicker("Welcome to Keepr", color = p.keep)
                Heading("Choose your language")
            }
        }
        KeeprText(
            "Keepr follows your choice throughout the app. Unsupported device languages use English.",
        ...
        LazyColumn(
            ...
            items(KeeprLanguages, key = { it.code }) { item ->
                ...
                HardSurface(
                    ...
                    Row(Modifier.fillMaxSize(), verticalAlignment = Alignment.CenterVertically) {
                        Column(Modifier.weight(1f)) {
                            KeeprText(item.native, color = p.textStrong, fontWeight = FontWeight.ExtraBold, fontSize = 16.sp)
                            Kicker(item.label)
                        }
        ...
        Column(Modifier.fillMaxWidth().background(p.appRaised).padding(18.dp, 10.dp)) {
            KButton("Continue", {
                ...
                val choice = KeeprLanguages.first { it.code == selected }
...
private data class IntroPage(val kicker: String, val title: String, val body: String, val icon: Int, val color: (KeeprPalette) -> Color)
private val IntroPages = listOf(
    ...
    IntroPage("Safe by design", "Review before anything leaves", "Inspect every choice, then confirm with Android. Photos stay on this device and Keepr verifies each result.", R.drawable.ic_shield) { it.gone },
)

@Composable
fun KeeprOnboardingScreen(
    onContinue: () -> Unit,
    onPrivacy: () -> Unit,
    isAdsEnabled: IsAdsEnabledUseCase = koinInject(),
) {
    ...
    Column(Modifier.fillMaxSize().background(p.app).systemBarsPadding()) {
        ...
        HorizontalPager(pager, Modifier.weight(1f)) { page ->
            ...
            Column(Modifier.fillMaxSize().padding(horizontal = 28.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
                HardSurface(Modifier.size(140.dp), radius = KeeprDimens.radius2Xl, background = p.card, shadowX = 8.dp, shadowY = 10.dp) {
                ...
                KeeprText(item.body, Modifier.fillMaxWidth(), color = p.textMuted, fontSize = 15.sp, textAlign = TextAlign.Center)
            }
        }
        ...
        Column(Modifier.fillMaxWidth().background(p.appRaised).padding(18.dp, 10.dp)) {
            ...
            Box(Modifier.fillMaxWidth().clickable { events.loadEvents("intro_scr_privacy_clck"); onPrivacy() }.padding(8.dp), contentAlignment = Alignment.Center) {
                KeeprText("Privacy details", color = p.textMuted, fontSize = 13.sp, fontWeight = FontWeight.Bold)
            }
        }
...
fun MediaAccessScreen(
    ...
    Column(Modifier.fillMaxSize().background(p.app).systemBarsPadding().padding(22.dp), horizontalAlignment = Alignment.CenterHorizontally) {
        ...
        HardSurface(Modifier.size(126.dp), radius = KeeprDimens.radius2Xl, shadowX = 8.dp, shadowY = 10.dp) {
        ...
        Heading(if (controller.hasPartialAccess()) "You choose what Keepr sees" else "Your camera roll stays yours", Modifier.fillMaxWidth(), 30.sp, TextAlign.Center)
        Spacer(Modifier.height(14.dp))
        KeeprText(
            "Keepr reads only the photos Android allows so you can review them month by month. Photo content is never uploaded. Deletion always uses Android confirmation.",
            Modifier.fillMaxWidth(), color = p.textMuted, fontSize = 15.sp, textAlign = TextAlign.Center
        )
        Spacer(Modifier.height(18.dp))
        HardSurface(Modifier.fillMaxWidth().height(74.dp), radius = KeeprDimens.radiusMd, background = p.cardRaised, shadowX = 0.dp, shadowY = 0.dp, borderWidth = 2.dp, contentPadding = PaddingValues(14.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                KeeprText("On-device processing · no account · no photo upload", color = p.textBody, fontSize = 13.sp, fontWeight = FontWeight.Bold)
            }
        }
